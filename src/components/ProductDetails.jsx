import React, { useContext, useEffect, useState } from "react";
import { FirebaseContext } from "../context/Context";
import { doc, getDoc } from "firebase/firestore";
import { useParams } from "react-router-dom";

function ProductDetails() {
  const { firestore } = useContext(FirebaseContext);
  const [productData, setProductData] = useState();
  const [sellerDetails, setSellerDetails] = useState();
  const { productId } = useParams();

  useEffect(() => {
    const productDocRef = doc(firestore, "products", productId);

    getDoc(productDocRef)
      .then((productDoc) => {
        if (productDoc.exists()) {
          const sellerDocRef = doc(
            firestore,
            "users",
            productDoc.data().user_id
          );
          getDoc(sellerDocRef)
            .then((userDoc) => {
              if (userDoc.exists()) {
                setProductData(productDoc.data());
                setSellerDetails(userDoc.data());
              }
            })
            .catch((error) => {
              console.error("Error fetching user:", error);
            });
        }
      })
      .catch((error) => {
        console.log("Error fetching product:", error);
      });
  }, []);

  return (
    <div className="p-3">
      <div className="flex flex-col md:flex-row rounded-md h-auto md:h-[30rem]">
        {productData ? (
          <>
            {/* Product Image Section */}
            <div className="bg-black w-full md:w-3/4 rounded-t-md md:rounded-l-md md:rounded-t-none text-white mb-4 md:mb-0">
              <img
                src={productData.image}
                alt="product image"
                className="object-contain w-full h-[300px] md:h-full"
              />
            </div>

            {/* Product and Seller Details */}
            <div className="bg-gray-100 w-full md:w-1/4 p-3 rounded-b-md md:rounded-r-md md:rounded-b-none">
              {/* Product Details */}
              <div className="bg-white rounded-md mt-3 p-3">
                <div className="flex flex-col">
                  <h3 className="underline text-lg font-bold">
                    Product Details
                  </h3>
                  <span className="pl-2 mt-2">{productData.name}</span>
                  <span className="pl-2 mt-1">₹{productData.price}</span>
                  <span className="pl-2 mt-1">
                    Category: {productData.category}
                  </span>
                  <span className="pl-2 mt-1">
                    Description: {productData.description}
                  </span>
                </div>
              </div>

              {/* Seller Details */}
              <div className="bg-white rounded-md mt-5 p-3">
                <div className="flex flex-col">
                  <h3 className="underline text-lg font-bold">
                    Seller Details
                  </h3>
                  <span className="pl-2 mt-1">{sellerDetails?.name}</span>
                  <span className="pl-2 mt-1">
                    {sellerDetails?.phone_number}
                  </span>
                  <span className="pl-2 mt-1">{sellerDetails?.email}</span>
                </div>
              </div>
            </div>
          </>
        ) : (
          <div className="flex justify-center items-center h-[300px] md:h-full w-full">
            {/* Loading Spinner */}
            <svg
              aria-hidden="true"
              className="w-8 h-8 text-gray-200 animate-spin fill-gray-400"
              viewBox="0 0 100 101"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                fill="currentColor"
              />
              <path
                d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                fill="currentFill"
              />
            </svg>
            <span className="sr-only">Loading...</span>
          </div>
        )}
      </div>
    </div>
  );
}

export default ProductDetails;
